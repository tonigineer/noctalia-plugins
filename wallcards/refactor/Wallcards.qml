...
PanelWindow {
  ...
  // property int thumbnailRevision: 0dda
  ...
  Column {
    ...
    property var closeWindow: () => window.destroy()

    Keys.onPressed: (event) => KeyHandler.handleKey(event, {
      // [Qt.Key_A]: () => textA.visible = !textA.visible,
      // [Qt.Key_D]: () => textD.visible = !textD.visible,
      [Qt.Key_Q]: closeWindow
    })
